...
std::string statusPrompt =
    "Analyse the lyrics of the song provided and choose "
...
std::string particleSelectionPrompt =
    "Analyse the lyrics of the song provided and choose 1-3 particle effect from "
    "the following list,"
...
void setPaths() {
  ...
  particleListFilePath =
      (currentDirectory / "AiResources" / "AiParticleList.json").string();
...
std::string getModelDevice() {
  ...
  std::vector<std::string> devicePriority = {"NPU", "GPU", "CPU"};
  std::vector<int32_t> deviceAvailability = {0, 0, 0};
  std::string selectedDevice = "";
  
  // First pass: find highest priority device
  for (const auto& priorityDevice : devicePriority) {
    for (const auto& device : availableDevices) {
      if (device.find(priorityDevice) != std::string::npos) {
        selectedDevice = device;
        break;
      }
    }
    if (!selectedDevice.empty()) break;
  }
  
  // Fallback to first device if none of the priorities matched
  if (selectedDevice.empty()) {
    selectedDevice = availableDevices[0];
  }
  
  // Validate that the selected device can actually compile the model
  try {
    std::cout << "Testing device compatibility: " << selectedDevice << std::endl;
    // Just create a simple model to test compilation
    auto model = std::make_shared<ov::Model>(
      ov::OutputVector{}, ov::ParameterVector{});
    core.compile_model(model, selectedDevice);
  } catch (const ov::Exception& e) {
    std::cout << "Device " << selectedDevice << " failed compatibility test: " 
              << e.what() << std::endl;
